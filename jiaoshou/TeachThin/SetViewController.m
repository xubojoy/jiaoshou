...
-(void)loadData
{
    ...
    request.successGetData = ^(id obj){
        ...
        if ([code isEqualToString:@"01"]) {
            ...
            _Birthday = [_UserDict valueForKey:@"birth"];
            _realnamel = [_UserDict valueForKey:@"truename"];
            _userSex = [_UserDict valueForKey:@"sex"];
...
//            [cell.contentView addSubview:tf1];
            ...
            case 2:
        {
            ...
            tf2.text = [NSString stringWithFormat:@"%@",_realnamel];
            ...
            case 3:
        {
            switch (indexPath.row) {
                ...
                {
                    _celltitle.text = @"生日";
                  _cellcontent.text = [NSString stringWithFormat:@"%@",_Birthday];
    ...
    switch (indexPath.section) {
        ...
        case 3:
        {
            switch (indexPath.row) {
               case 0:
                {
                    __weak SetViewController *vc = self;
                    self.cdp = [[CustomActionSheet alloc] initWithView:nil AndHeight:240];
                    self.cdp.SureBtnPress = ^(){
                        [vc datechange];

                    };
                    
                    self.dp = [[UIDatePicker alloc]initWithFrame:CGRectMake(0, 50, WIDTH_VIEW(self.view), 100)];
                    ...
                    [self.cdp.backGroundView addSubview:self.dp];
         ...
         case 4:
        {
            self.cas = [[CustomActionSheet alloc] initWithView:nil AndHeight:240];
            //[self.cdp showInView:self.view];
            self.cdp.SureBtnPress = ^(){
                if (!_clan) {
                    _clan = @"满族";
            ...
            [self.cas.backGroundView addSubview:self.pv];
...
-(void)datechange
{
    ...
    NSString *dateString = [ManageVC DateStrFromDate:selectedDate Withformat:@"YYYY-MM-dd"];
        NSLog(@"%@",dateString);
    _Birthday = [NSString stringWithFormat:@"%@",dateString];
    
    NSLog(@"________________%@",_Birthday);
    NSIndexPath *indexPath_1=[NSIndexPath indexPathForRow:0 inSection:3];
...
-(void)rightClick:(id)sender
{
   ...
   // NSString * uid = [[NSUserDefaults standardUserDefaults] valueForKey:@"uid"];
    ...
    NSString * sid  = [NSString stringWithFormat:@"%@",_sid];
    NSString * realname = [NSString stringWithFormat:@"%@",tf2.text];
    NSString * birthday = [NSString stringWithFormat:@"%@",_Birthday];
    ...
    NSString * nation = [NSString stringWithFormat:@"%@",_clan];
    NSString * elevel = [NSString stringWithFormat:@"%@",_EnucationLevel];
    NSString * act = [NSString stringWithFormat:@"%@",_actstrength];
    NSString * eat = [NSString stringWithFormat:@"%@",_eating];
    NSString * case1 = [NSString stringWithFormat:@"%@",_case1];

    NSString *  allergy = [NSString stringWithFormat:@"%@",_allergy];

    NSString * speical = [NSString stringWithFormat:@"%@",_special];


    NSLog(@"%@,%@,%@",_Birthday,_userSex,_special);
    NSDictionary * postDict = [NSDictionary dictionaryWithObjectsAndKeys:uid,@"uid",realname,@"name",birthday,@"birth",sex,@"sex",nation,@"nation",elevel,@"culture",act,@"work", eat,@"diet",case1,@"medical",allergy,@"allergen",speical,@"special",sid,@"id_s", nil];
    ...
    request.successGetData = ^(id obj){
        ...
        if ([result isEqualToString:@"01"]) {
            ...
            //头像保存
            [ManageVC sharedManage].userImg = [obj valueForKey:@"picurl"];
            [[NSUserDefaults standardUserDefaults] setObject:[obj valueForKey:@"picurl"] forKey:@"picurl"];
            //昵称保存
            [ManageVC sharedManage].niceName= [obj valueForKey:@"truename"];
            [[NSUserDefaults standardUserDefaults] setObject:[obj valueForKey:@"truename"] forKey:@"nicename"];
