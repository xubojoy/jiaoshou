...
{
    int index=0;
    int n = _DataArr.count%2==0?(int)_DataArr.count/2:(int)(_DataArr.count/2+1);
    for(int i=0;i<n;i++){
        for(int j=0;j<2;j++){
            ...
            UIImageView *img = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH_VIEW(btn)-32, 0, 32, 16)];
...
-(void)SelectBtnPress:(UIButton *)btn
{
    if(btn.tag==0){
        ...
        if([[[_DataArr objectAtIndex:self.PlanTag-100] valueForKey:@"state"] isEqualToString:@"0"]){
            detailVC.isDone = NO;//正在执行计划
        }else  detailVC.isDone = YES;
