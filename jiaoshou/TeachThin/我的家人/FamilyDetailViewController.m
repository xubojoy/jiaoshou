...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self loadData];
    [userimgView setImageWithURL:[NSURL URLWithString:[_userDataDict valueForKey:@"picurl"]]];
    titleLable.text = [_userDataDict valueForKey:@"realname"];
    phoneLable.text = [_userDataDict valueForKey:@"phone"];
...
-(void)setShowCurrentDate
{
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:_startDate];
    strYear = [[Datetime GetYear:date] intValue];
    strMonth = [[Datetime GetMonth:date] intValue];
...
-(void)buttenTouchUpInsideAction:(id)sender{
    ...
    NSLog(@"%d-%d-%@",strYear,strMonth,dayArray[t]);
    RecipesViewController *shipuVC = [[RecipesViewController alloc]init];
    shipuVC.Senddatetime = [NSString stringWithFormat:@"%d-%d-%@",strYear,strMonth,dayArray[t]];
    [self.navigationController pushViewController:shipuVC animated:YES];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    Calendercell.selectionStyle = UITableViewCellSelectionStyleNone;
    [self setCell2View];
    return Calendercell;
}
