...
-(void)renderFriendWithBuddyInfo:(NSDictionary *)buddy{
    NSLog(@">>>>>>>>>>>>>>>buddy :%@",buddy);
    UIImage *placeholderImg = [UIImage imageNamed:@"chatListCellHead.png"];
    NSString *picUrl = [buddy valueForKey:@"picurl"];
    if ([picUrl isEqualToString:@""] || picUrl == nil) {
        self.userImg.image = placeholderImg;
    }else{
        [self.userImg setImageWithURL:[NSURL URLWithString:picUrl] placeholderImage:placeholderImg];
    }
    self.nameLable.text = [buddy objectForKey:@"realname"];
    self.phoneLable.text = [NSString stringWithFormat:@"手机号：%@",[buddy objectForKey:@"phone"]];
}
