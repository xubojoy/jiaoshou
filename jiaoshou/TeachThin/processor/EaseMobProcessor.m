...
#endif
    [[[EaseMob sharedInstance] chatManager] isAutoFetchBuddyList];
                       ...
                       message:(NSString *)message
{
    ...
    if (self.mvc) {
        [self.mvc setupUntreatedApplyCount];
        NSInteger unreadCount = [[[EaseMob sharedInstance].chatManager conversations] count]+[[[ApplyFriendControllerViewController shareController] dataSource] count];
        if (unreadCount > 0) {
            // 播放音频
...
//    [self.mvc networkChanged:connectionState];
}
...
-(void)loginStateChange:(NSNotification *)notification
{
    ...
    if (isAutoLogin || loginSuccess) {
        ...
        nav  = self.mvc.navigationController;
    }
