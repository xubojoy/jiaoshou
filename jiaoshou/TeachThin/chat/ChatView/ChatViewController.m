    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if ([model isSender]) {
                ...
                if (![[ManageVC sharedManage].userImg isEqualToString:@""] || [ManageVC sharedManage].userImg != nil) {
                    NSLog(@"________________________%@",[ManageVC sharedManage].userImg);
                    model.headImageURL = [NSURL URLWithString:[ManageVC sharedManage].userImg];
                }else{
//                    model.headImageURL = [[NSBundle mainBundle] URLForResource:@"chatListCellHead" withExtension:@"png"];
                    model.headImageName = @"chatListCellHead.png";
                }
            }else{
                
                if (![self.receiverImg isEqualToString:@""] || self.receiverImg != nil) {
                    model.headImageURL = [NSURL URLWithString:self.receiverImg];
                }else{
                    model.headImageName = @"chatListCellHead.png";
                }
            }
