...
//        
        ...
        messageStr = [NSString stringWithFormat:@"%@请求加你为好友", username];
        [self sendFriendApplyAtIndexPath:self.selectedIndexPath
                                 message:messageStr];
                           ...
                           message:(NSString *)message
{
    ...
    NSString *buddyName = [infoDic valueForKey:@"uid"];
    NSLog(@">>>>>>>>>>>>>tel>>>>>>>>>>>>%@",[MyMD5 md5HexDigest:buddyName]);
        if (buddyName && buddyName.length > 0) {
            [self showHudInView:self.view hint:@"正在发送申请..."];
            EMError *error;
            [[EaseMob sharedInstance].chatManager addBuddy:[MyMD5 md5HexDigest:buddyName] message:message error:&error];
            [self hideHud];
            if (error) {
