...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if(imgv && [ManageVC sharedManage].userImg){
        [imgv setImageWithURL:[NSURL URLWithString:[ManageVC sharedManage].userImg]];
        tf1.text = [ManageVC sharedManage].name;
        tf2.text = [ManageVC sharedManage].pwd;
    }else{
...
-(void)setlayout
{
    ...
    imgv.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"loginheader"]];
    imgv.image = [UIImage imageNamed:@"loginheader"];
    if([ManageVC sharedManage].userImg){
        [imgv setImageWithURL:[NSURL URLWithString:[ManageVC sharedManage].userImg]];
    }
    ...
    if ([username isEqualToString:@""]||[password isEqualToString:@""]) {
        ...
        request.successGetData = ^(id obj){
            ...
            if ([result isEqualToString:@"01"]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:[obj valueForKey:@"picurl"] forKey:@"userimg"];
                [ManageVC sharedManage].niceName= [obj valueForKey:@"truename"];///////////昵称
                [[NSUserDefaults standardUserDefaults] setObject:[obj valueForKey:@"truename"] forKey:@"nick"];
                ...
                NSLog(@"___________________%@,%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"userimg"],[[NSUserDefaults standardUserDefaults] objectForKey:@"nick"]);
        ...
        request.failureGetData = ^(void){
            [MBHUDView dismissCurrentHUD];
            UIAlertView* alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"网络不给力，请检查网络" delegate:nil cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
            [alert show];
        };
