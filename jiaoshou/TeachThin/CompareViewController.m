...
-(void)loadData
{
    ...
    request.successGetData = ^(id obj){
        ...
        if([[dict valueForKey:@"code"]isEqualToString:@"01"]){
            ...
            //更改图库图片
            NSString *imgStr = [_DataDict valueForKey:@"img"];
            NSURL *url = [NSURL URLWithString:imgStr];
            TukuImgView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:url]];
            NSLog(@"DataDict-----%@",_DataDict);
            if(_Foodtype==1){
                _DanpinId = [_DataDict valueForKey:@"id"];
            }
            [table reloadData];
        }else if ([[dict valueForKey:@"code"]isEqualToString:@"02"]){
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"未匹配到任何结果" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
        }
    };
