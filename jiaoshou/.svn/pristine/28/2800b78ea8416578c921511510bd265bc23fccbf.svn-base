...
+(NonetView *)shareNetView;
{
    if(!_netview){
        ...
        _netview.frame = CGRectMake( 0, 64,VIEW_WEIGHT, [UIScreen mainScreen].bounds.size.height-64);
        [_netview setlayOut];
    }
    return _netview;
}
...
-(void)setlayOut
{
    ...
    UIImageView *netImg = [[UIImageView alloc]initWithFrame:CGRectMake(VIEW_WEIGHT/2-60, y, 120, 120)];
    netImg.image = [UIImage imageNamed:@"断网"];
    [self addSubview:netImg];
    
    UILabel *lable1 = [[UILabel alloc]initWithFrame:CGRectMake(VIEW_WEIGHT/2-100, VIEW_MAXY(netImg)+15, 200, 27)];
    ...
    UILabel *lable2 = [[UILabel alloc]initWithFrame:CGRectMake(VIEW_WEIGHT/2-100, VIEW_MAXY(lable1), 200, 27)];
